...
import { AggregateRoot, FixedClock, InMemoryRepository, ValueObject, createDomainEvent, createEntityId, createKernelRuntime, createRequestContext, defineUseCase, err, isErr, isOk, ok, unwrap, } from "../src/index.js";
class Customer extends AggregateRoot {
    static create(id, name) {
        const customer = new Customer(id, { name });
        customer.record(createDomainEvent({
...
describe("@insighthunter/kernel", () => {
    ...
    it("compares value objects by stable property values", () => {
        expect(Address.create("1 Market", "San Francisco").equals(Address.create("1 Market", "San Francisco"))).toBe(true);
        expect(Address.create("1 Market", "San Francisco").equals(Address.create("2 Market", "San Francisco"))).toBe(false);
    });
