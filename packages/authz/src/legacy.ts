...
import type { AuthContext, OrgRole, Permission, SessionPrincipal, TenantContext } from "./types";
...
export async function loadTenantContext(
  ...
  const row = await db
    .prepare(
      `SELECT om.org_id as orgId, om.role as role FROM organization_members om WHERE om.user_id = ?1 AND om.org_id = ?2 AND om.status = 'active' LIMIT 1`,
    )
    .bind(userId, orgId)
...
export async function writeAuditLog(
  ...
  await db
    .prepare(
      `INSERT INTO audit_logs (id, org_id, user_id, action, resource_type, resource_id, ip_address, user_agent, metadata_json, created_at) VALUES (?1,?2,?3,?4,?5,?6,?7,?8,?9,datetime('now'))`,
    )
    .bind(
      crypto.randomUUID(),
      input.orgId,
