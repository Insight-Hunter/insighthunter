...
paymentRoutes.post("/", async (c) => {
  ...
  const inv = await c.env.DB.prepare(
    ...
    .bind(body.invoice_id, session.orgId)
    .first<{
      id: string;
      org_id: string;
      total_amount: number;
  ...
  if (["void", "draft"].includes(inv.status))
    return c.json({ error: "Cannot apply payment to a draft or voided invoice" }, 409);

  const remaining = Number.parseFloat((inv.total_amount - inv.amount_paid).toFixed(2));
  const applying = Math.min(body.amount, remaining); // cap at remaining balance
  const newPaid = Number.parseFloat((inv.amount_paid + applying).toFixed(2));
  ...
  return c.json({
    ...
    applying,
    newPaid,
    remaining: Number.parseFloat((inv.total_amount - newPaid).toFixed(2)),
