...
clientRoutes.get('/', async (c) => {
  ...
  const result = await c.env.DB.prepare(
    `SELECT * FROM clients WHERE org_id = ?1 ORDER BY name ASC`
  ).bind(session.orgId).all<ClientRow>();
...
clientRoutes.get('/:id', async (c) => {
  ...
  ).bind(c.req.param('id'), session.orgId).first<ClientRow>();
  ...
  `).bind(session.orgId, c.req.param('id')).first<{ total_invoices: number; lifetime_billed: number; lifetime_paid: number }>();
...
clientRoutes.post('/', async (c) => {
  const session = getSession(c.req.raw);
  if (!session) return c.json({ error: 'unauthorized' }, 401);
  const body = await c.req.json<{ name: string; email?: string; phone?: string; address?: string; notes?: string }>();
  ...
  `).bind(id, session.orgId, body.name.trim(), body.email ?? null,
      body.phone ?? null, body.address ?? null, body.notes ?? null).run();
...
clientRoutes.patch('/:id', async (c) => {
  ...
  if (body.email   !== undefined) { sets.push(`email = ?${p++}`);   binds.push(body.email ?? null); }
  if (body.phone   !== undefined) { sets.push(`phone = ?${p++}`);   binds.push(body.phone ?? null); }
  if (body.address !== undefined) { sets.push(`address = ?${p++}`); binds.push(body.address ?? null); }
  if (body.notes   !== undefined) { sets.push(`notes = ?${p++}`);   binds.push(body.notes ?? null); }
  if (sets.length === 1) return c.json({ error: 'Nothing to update' }, 400);

  binds.push(c.req.param('id'), session.orgId);
  await c.env.DB.prepare(`UPDATE clients SET ${sets.join(', ')} WHERE id = ?${p++} AND org_id = ?${p++}`)
    .bind(...binds).run();
...
clientRoutes.delete('/:id', async (c) => {
  ...
  ).bind(c.req.param('id'), session.orgId).first<{ n: number }>();
  if ((open?.n ?? 0) > 0) return c.json({ error: 'Client has open invoices — void or collect them first' }, 409);
