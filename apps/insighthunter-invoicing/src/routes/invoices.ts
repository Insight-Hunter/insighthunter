...
invoiceRoutes.get("/:id", async (c) => {
  ...
  `)
    ...
    .first<InvoiceRow & { client_name: string; client_email: string; client_address: string }>();
...
invoiceRoutes.post("/", async (c) => {
  ...
  const taxAmount = Number.parseFloat((subtotal * (taxRate / 100)).toFixed(2));
  ...
  const stmts = [
    ...
    ...body.line_items.map((l, i) =>
      ...
      `).bind(
        ...
        l.quantity,
        l.unit_price,
        Number.parseFloat((l.quantity * l.unit_price).toFixed(2)),
        i,
      ),
    ),
  ];
...
invoiceRoutes.patch("/:id", async (c) => {
  ...
  if (body.memo !== undefined) {
    sets.push(`memo = ?${p++}`);
    binds.push(body.memo);
  }
  if (body.tax_rate !== undefined) {
    sets.push(`tax_rate = ?${p++}`);
    binds.push(body.tax_rate);
  }

  binds.push(c.req.param("id"), session.orgId);
  await c.env.DB.prepare(
    `UPDATE invoices SET ${sets.join(", ")} WHERE id = ?${p++} AND org_id = ?${p++}`,
  )
    .bind(...binds)
    .run();
...
invoiceRoutes.post("/:id/send", async (c) => {
  ...
  await c.env.DB.prepare(
    `UPDATE invoices SET status = 'sent', updated_at = datetime('now') WHERE id = ?1`,
  )
    .bind(c.req.param("id"))
    .run();
...
invoiceRoutes.post("/:id/void", async (c) => {
  ...
  if (["paid", "void"].includes(inv.status))
    return c.json({ error: "Cannot void a paid or already voided invoice" }, 409);

  await c.env.DB.prepare(
    `UPDATE invoices SET status = 'void', updated_at = datetime('now') WHERE id = ?1`,
  )
    .bind(c.req.param("id"))
    .run();
