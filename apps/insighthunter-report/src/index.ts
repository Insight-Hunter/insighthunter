...
=======
      ...
      const balClass = Math.abs(td-tc) < 0.01 ? 'amount-pos' : 'amount-neg';
      return '<table><thead><tr><th>Code</th><th>Account</th><th>Type</th><th class="amount">Debits</th><th class="amount">Credits</th></tr></thead><tbody>'+rows+
        '<tr class="total"><td colspan="3">Total</td><td class="amount '+balClass+'">'+fmt(td)+'</td><td class="amount '+balClass+'">'+fmt(tc)+'</td></tr></tbody></table>';
    }
    function renderPnl(d) {
      let h = '<table><thead><tr><th>Account</th><th class="amount">Amount</th></tr></thead><tbody>';
      h += '<tr class="section-header"><td colspan="2">Revenue</td></tr>';
      (d.revenue ?? []).forEach(r => h += '<tr><td style="padding-left:2rem">'+r.name+'</td><td class="amount">'+fmt(r.amount)+'</td></tr>');
      h += '<tr class="subtotal"><td>Total Revenue</td><td class="amount amount-pos">'+fmt(d.total_revenue)+'</td></tr>';
      h += '<tr class="section-header"><td colspan="2">Expenses</td></tr>';
      (d.expenses ?? []).forEach(r => h += '<tr><td style="padding-left:2rem">'+r.name+'</td><td class="amount">'+fmt(r.amount)+'</td></tr>');
      h += '<tr class="subtotal"><td>Total Expenses</td><td class="amount amount-neg">'+fmt(d.total_expenses)+'</td></tr>';
      const nc = d.net_income >= 0 ? 'amount-pos' : 'amount-neg';
      return h + '<tr class="total"><td>Net Income</td><td class="amount '+nc+'">'+fmt(d.net_income)+'</td></tr></tbody></table>';
    }
    function renderBalanceSheet(d) {
      let h = '<table><thead><tr><th>Account</th><th class="amount">Balance</th></tr></thead><tbody>';
      ['assets','liabilities','equity'].forEach(s => {
        h += '<tr class="section-header"><td colspan="2">'+s.charAt(0).toUpperCase()+s.slice(1)+'</td></tr>';
        (d[s] ?? []).forEach(r => h += '<tr><td style="padding-left:2rem">'+r.name+'</td><td class="amount">'+fmt(r.balance)+'</td></tr>');
        h += '<tr class="subtotal"><td>Total '+s.charAt(0).toUpperCase()+s.slice(1)+'</td><td class="amount">'+fmt(d['total_'+s])+'</td></tr>';
      });
      return h + '</tbody></table>';
    }
    function renderCashFlow(d) {
      let h = '<table><thead><tr><th>Category</th><th class="amount">Amount</th></tr></thead><tbody>';
      ['operating','investing','financing'].forEach(s => {
        h += '<tr class="section-header"><td colspan="2">'+s.charAt(0).toUpperCase()+s.slice(1)+' Activities</td></tr>';
        (d[s] ?? []).forEach(r => h += '<tr><td style="padding-left:2rem">'+r.label+'</td><td class="amount">'+fmt(r.amount)+'</td></tr>');
        h += '<tr class="subtotal"><td>Net '+s.charAt(0).toUpperCase()+s.slice(1)+'</td><td class="amount">'+fmt(d['net_'+s])+'</td></tr>';
      });
