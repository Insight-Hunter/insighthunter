...
reconciliationRoutes.get('/', async (c) => {
  ...
  `).bind(session.orgId).all<{
    id: string; txn_date: string; description: string;
    amount: number; account_id: string; account_name: string; status: string;
  }>();
...
reconciliationRoutes.post('/match', async (c) => {
  ...
  if (!body.reconItemId || !body.journalLineId) return c.json({ error: 'reconItemId and journalLineId required' }, 400);

  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE reconciliation_items SET status = 'matched', journal_line_id = ?1, matched_at = datetime('now') WHERE id = ?2 AND org_id = ?3`)
      .bind(body.journalLineId, body.reconItemId, session.orgId),
    c.env.DB.prepare(`UPDATE journal_lines SET reconciled = 1 WHERE id = ?1`)
      .bind(body.journalLineId),
  ]);
...
reconciliationRoutes.post('/clear', async (c) => {
  ...
  await c.env.DB.prepare(`UPDATE reconciliation_items SET status = 'cleared', cleared_at = datetime('now') WHERE id = ?1 AND org_id = ?2`)
    .bind(body.id, session.orgId).run();
