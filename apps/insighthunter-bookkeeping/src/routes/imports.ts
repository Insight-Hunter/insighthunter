...
importRoutes.post("/upload", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    .bind(importId, session.orgId, session.userId, file.name, objectKey)
    .run();

  // Set KV status for fast polling
  await c.env.KV_IMPORT_STATUS.put(
    `import:${importId}`,
    JSON.stringify({ status: "queued", rowCount: 0 }),
    { expirationTtl: 3600 },
  );
...
importRoutes.get("/:id", async (c) => {
  ...
  // Fast path: KV
  const kv = (await c.env.KV_IMPORT_STATUS.get(`import:${importId}`, "json")) as {
    status: string;
    rowCount: number;
    error?: string;
  } | null;
...
importRoutes.post("/:id/commit", async (c) => {
  ...
  if (body.overrides?.length) {
    const stmts = body.overrides.map((o) =>
      c.env.DB.prepare(
        `UPDATE import_rows SET category = ?1 WHERE id = ?2 AND import_id = ?3 AND org_id = ?4`,
  ...
  for (const row of rows.results ?? []) {
    ...
    // Mark row committed
    stmts.push(
      c.env.DB.prepare(`UPDATE import_rows SET review_status = 'committed' WHERE id = ?1`).bind(
        row.id,
      ),
    );
    committed++;
  ...
  stmts.push(
    c.env.DB.prepare(
      `UPDATE import_sessions SET status = 'committed', updated_at = datetime('now') WHERE id = ?1`,
    ).bind(importId),
  );
