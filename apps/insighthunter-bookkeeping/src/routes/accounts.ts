...
accountRoutes.post("/", async (c) => {
  ...
  if (!body.code || !body.name || !body.type)
    return c.json({ error: "code, name, type required" }, 400);
  ...
  await c.env.DB.prepare(
    ...
    .bind(id, session.orgId, body.code, body.name, body.type)
    .run();
...
accountRoutes.patch("/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    `UPDATE accounts SET ${sets.join(", ")} WHERE id = ?${p++} AND organization_id = ?${p++}`,
  )
    .bind(...binds)
    .run();
