...
CREATE INDEX idx_bk_acct_org  ON bookkeeping_accounts(org_id);
CREATE UNIQUE INDEX idx_bk_acct_code ON bookkeeping_accounts(org_id, code) WHERE code IS NOT NULL;
...
CREATE INDEX idx_txn_org_date ON transactions(org_id, date);
...
-- R2 receipt/document attachments
