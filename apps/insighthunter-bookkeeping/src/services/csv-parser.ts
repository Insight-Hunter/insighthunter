...
function parseAmount(raw: string, debit: string, credit: string): number | undefined {
  ...
  const d = Number.parseFloat(debit.replace(/[^0-9.-]/g, ""));
  const cr = Number.parseFloat(credit.replace(/[^0-9.-]/g, ""));
...
export async function parseCsv(text: string): Promise<ParsedImportRow[]> {
  ...
  const idx = (candidates: string[]) =>
    candidates.reduce((f, c) => (f >= 0 ? f : headers.findIndex((h) => h.includes(c))), -1);
