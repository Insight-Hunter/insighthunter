...
deductionRoutes.get('/', async (c) => {
  ...
  const emp = await c.env.DB.prepare(`SELECT id FROM employees WHERE id = ?1 AND org_id = ?2`)
    .bind(empId, session.orgId).first();
...
deductionRoutes.post('/', async (c) => {
  ...
  const body = await c.req.json<{
    employee_id: string; type: string;
    amount: number; is_percent?: boolean; description?: string;
  }>();
  ...
  // Verify employee belongs to org
  const emp = await c.env.DB.prepare(`SELECT id FROM employees WHERE id = ?1 AND org_id = ?2`)
    .bind(body.employee_id, session.orgId).first();
  if (!emp) return c.json({ error: 'Employee not found' }, 404);
  ...
  `).bind(id, body.employee_id, body.type, body.amount,
      body.is_percent ? 1 : 0, body.description ?? null).run();
...
deductionRoutes.patch('/:id', async (c) => {
  ...
  let p = 1;
  if (body.amount !== undefined) { sets.push(`amount = ?${p++}`); binds.push(body.amount); }
  if (body.active !== undefined) { sets.push(`active = ?${p++}`); binds.push(body.active ? 1 : 0); }
...
deductionRoutes.delete('/:id', async (c) => {
  ...
  if (!session) return c.json({ error: 'unauthorized' }, 401);
  if (!['owner', 'admin'].includes(session.role)) return c.json({ error: 'forbidden' }, 403);
  await c.env.DB.prepare(`DELETE FROM employee_deductions WHERE id = ?1`).bind(c.req.param('id')).run();
  return c.json({ ok: true });
});
