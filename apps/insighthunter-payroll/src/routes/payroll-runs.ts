...
payrollRunRoutes.get("/:id", async (c) => {
  const session = getSession(c.req.raw);
  if (!session) return c.json({ error: "unauthorized" }, 401);
  const run = await c.env.DB.prepare(`SELECT * FROM payroll_runs WHERE id = ?1 AND org_id = ?2`)
...
payrollRunRoutes.post("/", async (c) => {
  ...
  if (!body.period_start || !body.period_end)
    return c.json({ error: "period_start and period_end required" }, 400);
  ...
  for (const emp of employees.results) {
    const deductions: { type: string; amount: number; is_percent: number }[] = JSON.parse(
      emp.deductions_json ?? "[]",
  ...
  stmts.unshift(
    ...
    `).bind(
      ...
      body.period_start,
      body.period_end,
      Number.parseFloat(totalGross.toFixed(2)),
      Number.parseFloat(totalNet.toFixed(2)),
  ...
  return c.json(
    { id: runId, status: "draft", totalGross, totalNet, employeeCount: employees.results.length },
    201,
  );
});
...
payrollRunRoutes.post("/:id/void", async (c) => {
  ...
  const run = await c.env.DB.prepare(
    `SELECT status FROM payroll_runs WHERE id = ?1 AND org_id = ?2`,
  )
    .bind(c.req.param("id"), session.orgId)
    .first<{ status: string }>();
  if (!run) return c.json({ error: "Not found" }, 404);
  if (run.status === "approved")
    return c.json({ error: "Cannot void an approved run — contact support" }, 409);

  await c.env.DB.prepare(`UPDATE payroll_runs SET status = 'void' WHERE id = ?1`)
    .bind(c.req.param("id"))
    .run();
