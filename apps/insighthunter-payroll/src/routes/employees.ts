...
employeeRoutes.get("/:id", async (c) => {
  const session = getSession(c.req.raw);
  if (!session) return c.json({ error: "unauthorized" }, 401);
  const row = await c.env.DB.prepare(`SELECT * FROM employees WHERE id = ?1 AND org_id = ?2`)
...
employeeRoutes.post("/", async (c) => {
  ...
  if (!["salary", "hourly"].includes(body.pay_type))
    return c.json({ error: "pay_type must be salary or hourly" }, 400);
...
employeeRoutes.patch("/:id", async (c) => {
  ...
  if (body.pay_rate !== undefined) {
    sets.push(`pay_rate = ?${p++}`);
    binds.push(body.pay_rate);
  }
  ...
  await c.env.DB.prepare(
    `UPDATE employees SET ${sets.join(", ")} WHERE id = ?${p++} AND org_id = ?${p++}`,
  )
    .bind(...binds)
    .run();
