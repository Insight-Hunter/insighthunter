...
export async function writePayrollJournalEntry(opts: {
  ...
  const liabilityAccount = await db.prepare(
  ...
  if (!expenseAccount || !cashAccount) {
    console.warn('[payroll-journal] Required accounts not found for org', orgId, '— skipping journal entry');
    return;
  }
  ...
  const stmts = [
    db.prepare(`INSERT INTO journal_entries (id, organization_id, memo, posted_at, created_at)
      VALUES (?1,?2,?3,?4,datetime('now'))`).bind(jeId, orgId, memo, postedAt),

    // Dr Payroll Expense (gross)
    db.prepare(`INSERT INTO journal_lines (id, journal_entry_id, account_id, debit, credit, memo)
      VALUES (?1,?2,?3,?4,0,?5)`).bind(crypto.randomUUID(), jeId, expenseAccount.id, grossPay, memo),

    // Cr Cash (net pay disbursed)
    db.prepare(`INSERT INTO journal_lines (id, journal_entry_id, account_id, debit, credit, memo)
      VALUES (?1,?2,?3,0,?4,?5)`).bind(crypto.randomUUID(), jeId, cashAccount.id, netPay, memo),
  ];
  ...
  if (liabilityAccount && taxes > 0) {
    stmts.push(
      db.prepare(`INSERT INTO journal_lines (id, journal_entry_id, account_id, debit, credit, memo)
        VALUES (?1,?2,?3,0,?4,?5)`).bind(crypto.randomUUID(), jeId, liabilityAccount.id, taxes, memo)
    );
  }
