...
function estimateFederalTax(
  ...
  for (const [limit, rate] of brackets as [number, number][]) {
    ...
    tax += taxable * rate;
    prev = limit;
    if (limit === Number.POSITIVE_INFINITY) break;
  }
...
export function calculatePayrollLine(input: PayrollLineInput): PayrollLineResult {
  ...
  const federalTax = Number.parseFloat(((annualFedTax / 365) * days).toFixed(2));

  // State withholding
  const stateRate = STATE_FLAT_RATE[state.toUpperCase()] ?? 0.05;
  const stateTax = Number.parseFloat((grossPay * stateRate).toFixed(2));

  // FICA
  const ssWageThisCheck = Math.min(grossPay, FICA_SS_WAGE_BASE);
  const socialSecurity = Number.parseFloat((ssWageThisCheck * FICA_SS_RATE).toFixed(2));
  const medRate = annualized > 200000 ? FICA_MED_RATE + ADDL_MED_RATE : FICA_MED_RATE;
  const medicare = Number.parseFloat((grossPay * medRate).toFixed(2));

  // Other deductions (health, 401k, garnishments)
  const otherDeductions = Number.parseFloat(
    deductions
      .reduce((sum, d) => {
