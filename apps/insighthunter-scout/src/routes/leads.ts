...
leads.post("/", async (c) => {
  const session = getSession(c.req.raw);
  if (!session) return c.json({ error: "unauthorized" }, 401);
  const body = await c.req.json<{ name: string; email?: string; phone?: string; source?: string }>();
  ...
  ).bind(id, session.orgId, body.name, body.email ?? null, body.phone ?? null, body.source ?? "manual", new Date().toISOString()).run();
...
leads.patch("/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    `UPDATE leads SET status = ?1 WHERE id = ?2 AND org_id = ?3`
  ).bind(status, c.req.param("id"), session.orgId).run();
