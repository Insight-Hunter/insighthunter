...
// ── Rate limiting ─────────────────────────────────────────────────────────────
// Uses the SESSIONS KV namespace so no extra binding is needed.
// Key: "rl:login:<ip>"  Value: attempt count (string)  TTL: 15 min window
const RL_WINDOW_SECONDS = 15 * 60;
const RL_MAX_ATTEMPTS   = 10;  // per IP per 15-minute window

async function checkRateLimit(kv: KVNamespace, ip: string): Promise<boolean> {
  const key   = `rl:login:${ip}`;
  const raw   = await kv.get(key);
  const count = raw ? parseInt(raw, 10) : 0;
  if (count >= RL_MAX_ATTEMPTS) return false; // blocked
...
function authPage(mode: "login" | "register", returnTo: string, tier: Tier, error?: string): Response {
  ...
  const altHref  = isLogin
    ? `/register?returnTo=${encodeURIComponent(returnTo)}&tier=${tier}`
  ...
  return html(
    `<main class="card">
      ...
      <form method="post" action="${action}">
        ...
        <label class="field">Password<input type="password" name="password"
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (method === "POST" && url.pathname === "/register") {
      ...
      const token = await signSession(
        { userId: id, email, name, orgName: orgName || name, role: "owner", tier,
    ...
    // ── GET /session — cookie-based read (browser JS) ─────────────────────────
    ...
    // ── GET /session/verify — Service Binding endpoint (insighthunter-dashboard)
    // Accepts Bearer token (from dashboard env.AUTH_SERVICE.fetch()).
    // Returns the Session shape the dashboard verifySession() expects.
