...
const USAGE_METERS = {
  payroll: { meterId: 'mtr_test_61VEMZ54T16CvSGWl41F3gri2YoH21qq', eventName: 'payroll_employee_count' },
...
app.use('*', cors({
  origin: (origin) => (origin && (origin.endsWith('.insighthunter.app') || origin === 'https://insighthunter.app')) ? origin : null,
...
app.post('/usage', async (c) => {
  const body = await c.req.json<{ stripeCustomerId: string; type: 'payroll' | 'pbx'; value: number }>().catch(() => null);
...
app.post('/checkout', async (c) => {
  ...
  if (body.plan === 'enterprise') {
    return c.json({ error: 'Enterprise is custom-quoted. Contact sales instead of checkout.', contactUrl: 'https://insighthunter.app/contact-sales' }, 400);
  }
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      if (orgId && plan) {
        ...
        ).bind(plan, session.customer as string, session.subscription as string, orgId).run();
    ...
    case 'customer.subscription.deleted': {
      ...
      if (row) {
        await c.env.DB.prepare(`UPDATE organizations SET plan = 'starter' WHERE id = ?`).bind(row.id).run();
        await c.env.KV_ENTITLEMENTS.put(row.id, JSON.stringify({ plan: 'starter', updatedAt: Date.now() }));
      }
      break;
    }
  }
...
app.post('/addon-checkout', async (c) => {
  const body = await c.req.json<{ orgId: string; email: string; addon: keyof typeof ADDON_PRICE_IDS; quantity?: number }>().catch(() => null);
