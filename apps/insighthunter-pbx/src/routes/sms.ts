...
sms.post("/send", async (c) => {
  ...
  const { to, body, fromNumber } = await c.req.json<{ to: string; body: string; fromNumber: string }>();
  ...
  await c.env.DB.prepare(
    `INSERT INTO sms_log (org_id, to_number, from_number, body, twilio_sid, created_at)
     VALUES (?1, ?2, ?3, ?4, ?5, ?6)`
  ).bind(session.orgId, to, fromNumber, body, json.sid ?? null, new Date().toISOString()).run();
