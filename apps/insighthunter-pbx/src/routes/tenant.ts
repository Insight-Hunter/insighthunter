...
async function telnyxPost<T = Record<string, unknown>>(path: string, body: unknown, key: string): Promise<{ data: T }> {
...
tenant.post('/onboard', async (c) => {
  ...
  try {
    ...
    const { data: conn } = await telnyxPost<{
    ...
    const { data: cred } = await telnyxPost<{ id: string }>(
