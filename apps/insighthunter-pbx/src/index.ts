...
app.get('/api/pbx/call-logs', async (c) => {
  ...
  return c.json({  results, page, limit, total: countRow?.total ?? 0 });
});
...
// ── WebRTC Token ──────────────────────────────────────────────────────────────
// ── WebRTC Token — two-step Telnyx telephony credential flow ─────────────────
// Step 1 (at provisioning): credential already created, ID stored in orgs.telnyx_credential_id
// Step 2 (each WebRTC session): POST /telephony_credentials/{id}/token → short-lived JWT
app.post('/api/pbx/webrtc-token', async (c) => {
  const orgId = c.get('orgId');

  const org = await c.env.PBX_DB
    .prepare('SELECT telnyx_credential_id FROM orgs WHERE org_id=? AND status=\'active\' LIMIT 1')
    .bind(orgId).first<{ telnyx_credential_id: string }>();

  if (!org?.telnyx_credential_id)
    return c.json({ error: 'WebRTC not provisioned for this org' }, 404);

  // Exchange stored credential ID for a short-lived JWT token
  const r = await fetch(
  ...
  // Telnyx returns raw JWT string (not JSON)
  const token = await r.text();
  return c.json({ token: token.replace(/^"|"$/g, '') });
});
