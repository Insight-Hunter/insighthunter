...
// ── Health ────────────────────────────────────────────────────────────────────
app.get("/health", (c) => c.json({ ok: true, service: "insighthunter-advisor", env: c.env.ENVIRONMENT }));
// ═══════════════════════════════════════════════════════════════════════════════
// FIRMS
// ═══════════════════════════════════════════════════════════════════════════════
...
app.patch("/api/firms/:firmId", async (c) => {
    ...
    if (body.plan) {
        sets.push("plan = ?");
        vals.push(normalizePlan(body.plan));
    }
...
app.get("/api/firms/:firmId/members", async (c) => {
    const { userId } = c.get("auth");
    const firmId = c.req.param("firmId");
    if (!(await firmAccess(c.env.DB, firmId, userId)))
        return c.json({ error: "forbidden" }, 403);
    const { results } = await c.env.DB.prepare(`
...
app.patch("/api/firms/:firmId/clients/:clientId", async (c) => {
  ...
  `)
        .bind(body.assigned_staff_user_id ?? null, body.status ? normalizeClientStatus(body.status) : null, firmId, clientId)
        .run();
...
app.post("/api/firms/:firmId/alerts", async (c) => {
    ...
    if (!body.alert_type || !body.title)
        return c.json({ error: "alert_type and title required" }, 400);
    const id = uuid();
    const now = epoch();
    await c.env.DB.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
        .bind(id, firmId, body.business_id ?? null, body.alert_type, normalizeSeverity(body.severity), body.title, body.body ?? null, now)
        .run();
...
app.patch("/api/firms/:firmId/clients/:clientId/notes/:noteId", async (c) => {
  ...
  `)
        .bind(body.body?.trim() ?? null, body.pinned !== undefined ? (body.pinned ? 1 : 0) : null, noteId, firmId, userId)
        .run();
    return c.json({ ok: true });
});
...
async function resolveAuth(request) {
    ...
    if (demoId)
        return { userId: demoId, email: request.headers.get("x-demo-email") ?? undefined };
...
async function firmAccess(db, firmId, userId) {
    return db
        .prepare(`SELECT firm_id, user_id, role FROM firm_members WHERE firm_id = ? AND user_id = ?`)
        .bind(firmId, userId)
        .first();
}
async function firmRole(db, firmId, userId, roles) {
    const m = await firmAccess(db, firmId, userId);
    return m && roles.includes(String(m.role)) ? m : null;
}
function normalizePlan(v) {
    const s = (v ?? "starter").toLowerCase();
    return ["starter", "pro", "enterprise"].includes(s) ? s : "starter";
}
function normalizeRole(v) {
    const s = (v ?? "staff").toLowerCase();
    return ["owner", "admin", "staff", "viewer"].includes(s) ? s : "staff";
}
function normalizeClientStatus(v) {
    const s = (v ?? "active").toLowerCase();
    return ["active", "inactive", "offboarded"].includes(s) ? s : "active";
}
...
function track(env, event, userId, firmId, meta) {
