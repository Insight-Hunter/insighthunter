...
const ALL_APPS: AppDef[] = [
  ...
  },
  {
    slug: "reports",
    name: "Reports",
    description: "Automated financial reports & exports",
  ...
  },
  {
    slug: "scout",
    name: "Scout",
    description: "Business intelligence & market signals",
  ...
  },
  {
    slug: "finops",
    name: "FinOps",
    description: "Cloud cost optimization & tracking",
  ...
  },
  {
    slug: "dispatch",
    name: "Dispatch",
    description: "Operations & task dispatch",
  ...
  },
  {
    slug: "notifications",
    name: "Notifications",
    description: "Alerts & team notifications",
...
dashboardRoutes.get("/", async (c) => {
  ...
  const notifResult = await c.env.DB.prepare(`SELECT id, title, body, type, read, created_at
              FROM notifications
  ...
  const auditResult = await c.env.DB.prepare(`SELECT action, resource_type, created_at
              FROM audit_logs
              WHERE org_id = ?1
              ORDER BY created_at DESC LIMIT 5`)
    .bind(orgId)
    .all<{ action: string; resource_type: string; created_at: string }>();
...
dashboardRoutes.get("/apps", async (c) => {
  ...
  return c.json({
    apps: ALL_APPS.filter((a) => a.plans.some((p) => (PLAN_RANK[p] ?? 0) <= userRank)),
  });
});
