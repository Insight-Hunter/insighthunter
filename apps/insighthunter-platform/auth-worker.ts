  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/entitlements" && request.method === "GET") {
        return withCors(await handleEntitlements(request, env), cors);
      }
...
/** Returns the user's account tier plus any active module add-on
 * entitlements — used by insighthunter-dashboard and every module worker
 * to decide what to render/allow. Requires a valid session. */
async function handleEntitlements(request: Request, env: Env): Promise<Response> {
  const token = bearerToken(request);
  if (!token) return Response.json({ error: "missing_token" }, { status: 401 });
  ...
  const addons = await env.DB.prepare(
    "SELECT module, tier, status FROM entitlements WHERE user_id = ? AND status = 'active'"
  )
    .bind(payload.userId)
    .all<{ module: string; tier: string; status: string }>();
