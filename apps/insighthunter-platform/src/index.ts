...
async function writeAudit(
  ...
  await db
    ...
    .bind(crypto.randomUUID(), orgId, userId, action, resourceType, resourceId, ip)
    .run();
}
...
app.get("/settings", async (c) => {
  ...
  if (!session)
    return c.redirect(
      c.env.AUTH_URL +
        "/login?redirect=" +
        encodeURIComponent("https://platform.insighthunter.app/settings"),
    );
...
app.post("/settings", async (c) => {
  ...
  await c.env.DB.prepare("UPDATE organizations SET name = ?, slug = ?, updated_at = ? WHERE id = ?")
    .bind(name, slug, Math.floor(Date.now() / 1000), session.orgId)
    .run();
  await writeAudit(
    c.env.DB,
    session.orgId,
...
app.post("/settings/transfer", async (c) => {
  ...
  const newOwner = await c.env.DB.prepare(
    ...
    .bind(session.orgId, email)
    .first<{ id: string }>();

  if (!newOwner) return c.redirect("/settings?error=not_found");

  await c.env.DB.batch([
    c.env.DB.prepare("UPDATE org_members SET role = 'admin' WHERE org_id = ? AND user_id = ?").bind(
      session.orgId,
      session.userId,
    ),
    c.env.DB.prepare("UPDATE org_members SET role = 'owner' WHERE org_id = ? AND user_id = ?").bind(
      session.orgId,
      newOwner.id,
    ),
    c.env.DB.prepare("UPDATE organizations SET owner_id = ?, updated_at = ? WHERE id = ?").bind(
      newOwner.id,
...
app.get("/members", async (c) => {
  ...
  if (!session)
    return c.redirect(
      c.env.AUTH_URL +
        "/login?redirect=" +
        encodeURIComponent("https://platform.insighthunter.app/members"),
    );
        ...
        <tbody>
          ${members.results?.map(
            ...
            <tr>
              ...
              ${
                isAdminPlus && m.user_id !== session.userId
                  ? html`
...
app.post("/api/members/invite", async (c) => {
  ...
  if (session.role !== "owner" && session.role !== "admin")
    return c.redirect("/members?error=forbidden");
...
app.get("/billing", async (c) => {
  ...
  if (!session)
    return c.redirect(
      c.env.AUTH_URL +
        "/login?redirect=" +
        encodeURIComponent("https://platform.insighthunter.app/billing"),
    );
  ...
  return c.html(
    ...
    <div class="card">
      ...
      <table>
        ...
        <tbody>
          ${Object.entries(PLAN_FEATURES[currentPlan]).map(
            ([key, val]) => html`
            <tr>
              ...
              <td>${
                typeof val === "boolean"
                  ? val
                    ...
                    : html`<span style="color:var(--muted)">❌ Upgrade required</span>`
...
app.get("/audit", async (c) => {
  ...
  if (!session)
    return c.redirect(
      c.env.AUTH_URL +
        "/login?redirect=" +
        encodeURIComponent("https://platform.insighthunter.app/audit"),
    );
  if (session.role !== "owner" && session.role !== "admin")
    return c.redirect("/settings?error=forbidden");
  ...
  const rows = await c.env.DB.prepare(
    ...
    .bind(session.orgId, limit, offset)
    .all<AuditRow>();

  const total = await c.env.DB.prepare("SELECT COUNT(*) as n FROM audit_logs WHERE org_id = ?")
    .bind(session.orgId)
    .first<{ n: number }>();
...
app.get("/api/members", async (c) => {
  ...
  const members = await c.env.DB.prepare(
    ...
    .bind(session.orgId)
    .all<{ id: string; user_id: string; role: string; email: string; name: string }>();
